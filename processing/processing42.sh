#ubuntu 22.04.01 LTS + processing 4.2 Linux 64bits


if [[ $SUDO_USER ]] ; then
  echo "Just use 'bash processing42.sh'"
  exit 1
fi
...
sudo tar -xvf processing-4.2-linux-x64.tgz
sudo mv ./processing-4.2 ./processing
sudo mv ./processing /opt/.
sudo bash /opt/processing/install.sh
sudo rm processing-4.2-linux-x64.tgz
